...
const Question = () => {
    ...
    const { searchTerm: hookSearchTerm, setSearchTerm: setHookSearchTerm, filteredData } = useSearch(allQuestions, ['content']);

    // Sort functionality  
    const { sortedData, sortKey: hookSortKey, sortOrder: hookSortOrder, handleSort } = useSort(filteredData, 'content', 'asc');
    ...
    const getFilteredAndSortedQuestions = () => {
        ...
        if (levelFilter !== 'all') {
            filtered = filtered.filter(question => question.difficultyLevel === levelFilter);
        }
    ...
    const { currentPage, totalPages, paginatedData, goToPage, nextPage, prevPage, hasNext, hasPrev } = usePagination(filteredQuestions, 10);
            ...
            <div className="card">
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    <div className="flex flex-col sm:flex-row gap-4 flex-1">
                        ...
                        {activeTab === 'questions' && (<div className="flex gap-3">
                            <select
                                value={statusFilter}
                ...
                <>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {questionFiles.map((file) => (
                            <div key={file.id} onClick={() => window.open(file.url, '_blank')} className="card hover:shadow-lg transition-shadow">
                    ...
                    {questionFiles.length === 0 && !loading && (
                        <div className="text-center py-12">
                            ...
                            <h3 className="text-lg font-medium text-gray-900 mb-2">No question files found</h3>
                            <p className="text-gray-600 mb-6">Upload your first question file to get started.</p>
                            <button
                                onClick={handleFileUpload}
...

            {
                showViewModal && viewingQuestion && (
                    <div className="modal-overlay bg-black bg-opacity-50 flex items-center justify-center">
                        <div className="modal-content bg-white rounded-lg shadow-lg w-full max-w-2xl">
                            <div className="p-6">
                                <div className="flex items-center justify-between mb-6">
                                ...
                                <div className="space-y-4">
                                    <div>
                                        <label className="block text-sm font-semibold text-gray-600 mb-1">Question Content</label>
                                        <p className="text-lg font-medium text-gray-900 leading-relaxed">{viewingQuestion.content}</p>
                                    </div>
                                    <div>
                ...
                showFileModal && (
                    <div className="modal-overlay">
                        <div className="modal-content">
                            <div className="p-6">
                                <div className="flex items-center justify-between mb-6">
                                    <h3 className="text-xl font-bold text-gray-900">Upload Question File</h3>
                                    <button
